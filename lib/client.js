...
module.exports = {
   ...
   */
  originalRequest: function (param, retry, callback) {
  ...
  request: function (param, retry, callback) {
    ...
    while (a === 1) {
      try {
        result = this.originalRequest(param, retry, callback);
        if (result.statusCode.match(/^40\d$/)) {
          throw new Error(`エラー： ${result.statusCode}`);
        } else if (result.statusCode.match(/^50\d$/)) {
          throw new Error(`エラー： ${result.statusCode}`);
        } else if (result.body.match(/只今(?:ｱｸｾｽ|アクセス)が集中/)) {
          throw new Error('エラー： アクセス集中');
        } else {
          return result;
        }
      } catch (error) {
